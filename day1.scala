...
def massCalculator(moduleMass: Int): Int = {
    def internalMassCalculator(metaFuelMass: Int, totalFuelMass: Int, depth: Int): Int = {
        val newMetaFuelMass = (metaFuelMass)/3 - 2
        if (newMetaFuelMass <= 0) totalFuelMass 
        else internalMassCalculator(newMetaFuelMass, totalFuelMass+newMetaFuelMass, depth + 1)
    }
